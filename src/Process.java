...
public class Process {
    ...
    public Process(String id, int port, int numID) throws IOException {
        this.id = id;
        this.ownPort = port;
        this.numID = numID;
    ...
    public void broadcastMessage(List<String> receivers, int id)
        ...
        while (this.messageSent < 100 || this.messageReceived < 300) {
            Thread.sleep(100);
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 1; i < 5; i++) {
            if (i != id) {
                String IP = "10.176.69." + (31 + i);
                receivers.add(IP + ":" + (2100 + i));
            }
        }

        Process process = new Process(processId, port, id);
        Thread.sleep(2000);
        ...
        while (process.connections.size() < 3) {
            Thread.sleep(100);
        }
        Thread.sleep(1000);
